...
class FourLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        #y_label = np.zeros((N,class_probs.shape[1]))
        #y_label[np.arange(N),y] = 1
        
        #data_loss = (-1) * np.sum(np.multiply(np.log(probs),correct_logprobs)) / N
        #loss = (-1) * np.sum(np.multiply(np.log(probs),y_label)) / N
        data_loss = np.sum(correct_logprobs)/N
        reg_loss = reg * (np.sum(W1**2) + np.sum(W2**2)+ np.sum(W3**2)+ np.sum(W4**2))
        #loss = (-1) * np.sum(np.multiply(np.log(class_probs),y_label)) / N
        ...
        dZ3 = dZ4.dot(W4.T) * (h3 > 0)
        dW3 = h2.T.dot(dZ3) + reg * W3
