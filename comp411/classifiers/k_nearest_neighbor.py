...
class KNearestNeighbor(object):
    ...
    def compute_L2_distances_no_loops(self, X):
        ...
        X_train2 = np.sum(self.X_train**2, axis = -1)[:, np.newaxis]
        X_test2 = np.sum(X**2, axis = -1)[:, np.newaxis]
        X_tr_test = X @ self.X_train.T
            
        dists = np.sqrt(X_test2 + X_train2.T - 2*X_tr_test)
    ...
    def compute_Cosine_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                dists[i][j] = np.sum(np.dot(X[i],self.X_train[j])) / (np.sqrt(np.sum(np.square(X[i]))) * np.sqrt(np.sum(np.square(self.X_train[j]))))
    ...
    def compute_Cosine_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            dists[i, :] = np.dot(X[i],self.X_train.T) / (np.sqrt(np.sum(np.square(X[i]))) * np.sqrt(np.sum(np.square(self.X_train), axis = 1)))
            # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return dists
    ...
    def compute_Cosine_distances_no_loops(self, X):
        ...
        dists = np.dot(X, self.X_train.T) / (np.sqrt(np.sum(np.square(X), axis = 1))[:, np.newaxis] * np.sqrt(np.sum(np.square(self.X_train), axis = 1)))
